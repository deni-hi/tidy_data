...
# that should result in 561 columns, so each element in a line corresponds to one of these features
...
# each measurement.
library("dplyr")
meanAndStandardDev<-select(mergedData, tBodyAcc.mean...X:tBodyAcc.std...Z, activity, subject)
str(meanAndStandardDev)
...
translateActivity<-function(x){
    activityLabels$label[x]
}
meanAndStandardDev$activity<-sapply(meanAndStandardDev$activity, translateActivity)

# STEP 4 - Appropriately label the data set with descriptive variable names.
names(meanAndStandardDev)<-c("meanX","meanY","meanZ",
                             "stdX","stdY","stdZ", 
                             "activity", "subject")

# STEP 5 - From the data set in step 4, create a second, independent tidy data 
# set with the average of each variable for each activity and each subject.
by_activity <- group_by(meanAndStandardDev, activity, subject)

avgSummary<-summarise(
    by_activity, 
    avg_meanX=mean(meanX), avg_meanY=mean(meanY), avg_meanZ=mean(meanZ),
    avg_stdX=mean(stdX), avg_stdY=mean(stdY), avg_stdZ=mean(stdZ))
